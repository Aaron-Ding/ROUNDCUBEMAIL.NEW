...
*/
  ...
  function load_settings()
  {
    ...
    if ($this->ui->screen == 'calendar') {
      ...
      if (isset($_SESSION["plugin.rcp_public_mailbox.rawuser"]) && $_SESSION["plugin.rcp_public_mailbox.rawuser"]!="" && $_SESSION["username"]!=$_SESSION["plugin.rcp_public_mailbox.rawuser"] ) {
          $real_user=$_SESSION["plugin.rcp_public_mailbox.rawuser"];
          $real_rec=$_SESSION["plugin.rcp_public_mailbox.list"][$_SESSION["plugin.rcp_public_mailbox.rawuser_md5"]];
          $settings['identities']=array();
          $settings['identities'][$real_rec['userid']] = $real_user;

          $settings['identity'] = array('name' => current(explode('@',$real_user)), 'email' => strtolower($real_user), 'emails' => ';' . strtolower(join(';', array($real_user))));
      }else{
...
//      $chars = "!# abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 1234567890";
  ...
  private function write_preprocess(&$event, $action)
  {
    ...
    if ($action == 'new' || $action == 'edit') {
      if (!$event['attendees']){
          $real_user =$_SESSION["username"];
          if (isset($_SESSION["plugin.rcp_public_mailbox.rawuser"]) && $_SESSION["plugin.rcp_public_mailbox.rawuser"]!="" && $_SESSION["username"]!=$_SESSION["plugin.rcp_public_mailbox.rawuser"] ) {
              $real_user = $_SESSION["plugin.rcp_public_mailbox.rawuser"];

          }
          $event['attendees'] = array();
          $event['attendees'] []=array('role'=>"ORGANIZER",'name' => current(explode('@', $real_user)), 'email' => strtolower($real_user), 'emails' => ';' . strtolower(join(';', array($real_user))));
