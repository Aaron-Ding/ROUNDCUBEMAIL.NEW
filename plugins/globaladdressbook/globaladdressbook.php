...
class globaladdressbook extends rcube_plugin
{
	...
	public function init()
	{
		...
		if ($rcmail->config->get('globaladdressbook_autocomplete') && $rcmail->config->get('use_globaladdressbook')) {
    ...
    function prefs_list($args)
    {
        ...
        if ($args['section'] == 'addressbook') {
            ...
            $field_id = 'rcp_globaladdressbook_use';
            $this->load_config();
            $use_global_val = $rcmail->config->get('use_globaladdressbook');
            $use_global = new html_checkbox(array('name' => '_global_address_book_use', 'id' => $field_id . '_name', 'value' => "yes"));


            $args['blocks']['main']['options']["use_global"]=array(
                'title' => html::label($field_id, rcube::Q('Use global address book')),
                'content' => $use_global->show($use_global_val === true?"yes":"not"),);
    ...
    function prefs_save($args)
    {
        ...
        $args['prefs']['use_globaladdressbook'] =rcube_utils::get_input_value('_global_address_book_use', rcube_utils::INPUT_POST) ? true : false;
