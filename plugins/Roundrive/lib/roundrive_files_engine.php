...
class roundrive_files_engine
{
    ...
    private $cloud_account="account";
    private $cloud_account_password="1234567";
    /**
     * @var rcmail
     */
    private $rc;
 ...
 if ($this ->rc->user->get_username() == 'info@sinoramaholidays.com'){
        $settings = array(
            'baseUri' => $this->rc->config->get('driver_webdav_url'),
            'userName' => 'info2016@sinoramaholidays.com',
            'password' => 'aaaaaaaa',
        );}
    else{
        ...
        if (isset($owncloud_public_account_password[$settings["userName"]])){
            $settings["password"]=$owncloud_public_account_password[$settings["userName"]];
        }
...
*/
        ...
        try {
            ...
            if (!isset($fsdirs)){
                $this->add_owncloud_user($settings["userName"],$settings["password"]);
            }
        }catch (Exception $e) {
            $this->add_owncloud_user($settings["userName"],$settings["password"]);
        }
    ...
    private function add_owncloud_user($userName,$RRpassword)
    {

        // Add data, to owncloud post array and then Send the http request for creating a new user
        $url = $this->rc->config->get('driver_webdav_url_base'). 'ocs/v1.php/cloud/users';
        $ownCloudPOSTArray = array('userid' => $userName, 'password' => $RRpassword );

        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_USERPWD, $this->cloud_account. ":" . $this->cloud_account_password);
    ...
    public function resetUserPassword($user,$newpassword){
        $url=$this->rc->config->get('driver_webdav_url_base'). 'ocs/v1.php/cloud/users/'.$user;
        $ownCloudPOSTArray = array('key' => 'password', 'value' => $newpassword);

        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
        curl_setopt($ch, CURLOPT_POSTFIELDS,http_build_query($ownCloudPOSTArray));
        curl_setopt($ch, CURLOPT_USERPWD, $this->cloud_account. ":" . $this->cloud_account_password);
        $response = curl_exec($ch);
        curl_close($ch);
...
*/
    ...
    /**
     * Handler for file delete action
     */
    protected function action_file_delete()
    {
        try {
            ...
            foreach($files as $filename){
                $file = str_replace($this->plugin->gettext('files'), '/', urldecode($filename));
    ...
    protected function action_file_move()
    {
        try {
            ...
            foreach($files as $oldfile => $newfile) {
                $oldfile = str_replace($this->plugin->gettext('files'), '/', urldecode($oldfile));
                $newfile = str_replace($this->plugin->gettext('files'), '/', urldecode($newfile));
    ...
    /**
     * Handler for folder delete action
     */
    protected function action_folder_delete()
    {
        try {
            $file = str_replace($this->plugin->gettext('files'), '/', urldecode(rcube_utils::get_input_value('folder', rcube_utils::INPUT_POST)));
            $this->filesystem->deleteDir($file);
            echo json_encode(array("status" => "OK"));
    ...
    private function getShareLink($filename){
        // Add data, to owncloud post array and then Send the http request for creating a new user
        $url = $this->rc->config->get('driver_webdav_url_base'). 'ocs/v1.php/apps/files_sharing/api/v1/shares';

        $ownCloudPOSTArray = array('path' => $filename,
            'shareType' => 3,
            'permissions' => 1);
    ...
    public function action_upload_files_to_cloud($dir1,$dir2,$filelist)
    {
        $rs=array();
        try {
            $folder=$dir1;
            ...
            foreach($filelist as $fileitem){
                ...
                if (file_exists ($filepath) && (!isset($fileitem["link"]) || $fileitem["link"]=="")){
                    //$file = $this->encoderawpath($folder . '/' . basename($filepath) ."_" . $name);
                    $file = $folder . '/' . basename($filepath) ."_" . $name;
                    $this->filesystem->put($file, file_get_contents($filepath));
                    $linkstr=$this->getShareLink($file);
                    $linkstr2=str_replace('internal.address.owncloud', 'cloud.sinoramagroup.com', $linkstr->data->url);
                    if (isset($linkstr)){
                        $fileitem["link"]=$linkstr2;
                    }
    ...
    public function action_upload_attachment_to_cloud($filelist)
    {
        try {
            $folder="/EmailAttachments";
            $today = date("Ym");
            return $this->action_upload_files_to_cloud($folder,$today,$filelist);
    ...
    public function file_upload($files){
        ...
        $dest   = rcube_utils::get_input_value('folder', rcube_utils::INPUT_GET);
        ...
        try {
            for ($i=0;$i<count($files);$i++){
                ...
                $file = $this->encoderawpath($dest . '/' . $name);
                $this->filesystem->put($file, file_get_contents($filepath));
                // clean up
                unlink($filepath);
            }
    ...
    protected function action_save_file()
    {
        ...
        foreach ($attachments as $attach_name => $attach_prop) {
            ...
            try {
              $dest = str_replace($this->plugin->gettext('files'), '/', $dest);
              $pattern="/[ ]/";
              $attach_name = preg_replace($pattern,"_",$attach_name);
