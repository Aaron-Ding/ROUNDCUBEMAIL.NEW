...
window.rcmail && window.files_api && rcmail.addEventListener('init', function() {
  if (rcmail.task == 'mail') {
    ...
    if (rcmail.env.action == 'compose') {
      ...
      rcmail.check_attchemnts=function(mailid){

          var data={_id:mailid,return_type:"JSON"};
          $.ajax({
              type: 'POST', url: rcmail.url('roundrive/check_attchment_size'), data:data , dataType: 'json',
              success: function(response) {


                  if (response.error==undefined ){
                      rcmail.display_message(response.msg, "notice");
                      rcmail.add_cloud_attchments_to_mail(response.files);
                  }else{
                      rcmail.display_message(response.msg, "error");
                  }

              },
              error: function(o, status, err) { console.log(o, status, err); },
              cache: false,
          });
      }
      rcmail.add_cloud_attchments_to_mail=function(atachments_list){
          var messageEditor=this.editor;
          var fileinfoJSON=atachments_list[1].replace(/\\"/g,'"').replace(/\//g,'/');
          var fileinfo=JSON.parse(atachments_list);
          messageEditor.add_bigattachment(fileinfo);
      }
      rcmail.editor.add_bigattachment= function(files)
      {
          if (this.editor) { // html mode
              var tmplatestr='<p>&nbsp;</p><table width="100%">'+
                  '<tbody>'+
                  '<tr>'+
                  '<td  height="30px"><b>Large attachments from SINORAMA mail</b></td>'+
                  '</tr>';

              files.forEach(function(file_para){
                  tmplatestr+='<tr>'+
                      '<td bgcolor="#e0ecf9" height="30px"><a style="color: #000;" href="'+file_para.link+'" target="_blank" rel="noreferrer">'+file_para.name+'</a>('+file_para.size+')&nbsp;&nbsp;<a href="'+file_para.link+'" target="_blank" rel="noreferrer" style="padding-left: 30px;">download</a></td>'+
                      '</tr>';
              });
              tmplatestr+='</table>';
              ...
              var bigElem=doc.getElementById('sinoramabigattachment');
              if (!bigElem){
                  bigElem = doc.createElement('div');
                  bigElem.id='sinoramabigattachment';
                  if (!sigElem) {
                      body.appendChild(bigElem);
                  }else{
                      body.insertBefore(bigElem, sigElem);
                  }
              }
              bigElem.innerHTML = tmplatestr;
          }else{ //text model
              input_message = $('#' + this.id);
              message = input_message.val();
              tmplatestr='\n\nLarge attachments from SINORAMA mail. Please copy the link to browser.\n';
              files.forEach(function(file_para){
                  tmplatestr+=file_para.name+'  ('+file_para.size+')   '+file_para.link+"\n";
              });
              tmplatestr+="\n\n"
              pos = rcmail.get_caret_pos(input_message.get(0));
...
*/
  ...
  this.folder_list_response = function(response)
  {
    ...
    var first, elem = $('#files-folder-list'),
      ...
      collections = !rcmail.env.action.match(/^(preview|show)$/) ? [] : [];
  ...
  this.file_upload_response = function(response)
  {
    if (response && response.status === "OK") {
      this.display_message(response.msg, 'confirmation');
    }
  ...
  this.file_upload_form = function(form, action, onload)
  {
    ...
    $(form).attr({
      target: frame_name,
     // action:  rcmail.url(action, {folder: rcmail.env.folder, token: rcmail.env.token}),
      action :file_api.url('file_upload', {folder: file_api.env.folder}),
      method: 'POST'
