...
//      monthNamesShort: settings['months'],
    ...
    function init()
    {
        ...
        .on('keydown', '.taskhead', function(e) {
            if (e.targettargettargettarget.nodeName == 'INPUT' && e.target.type == 'text')
                return true;
    ...
    function init_taskedit()
    {
        $('#taskedit').tabs({
            activate: function(event, ui) {
                ...
                if (ui.newPanel != undefined && ui.newPanel.attr('id') == "taskedit-panel-comments") {
                    display_comments();
                }
            }
        });
...
*/
    ...
    function task_show_dialog(id, data, temp)
    {
        ...
        function last_show_dialog(newrec){
            if (newrec) rec=newrec;
            me.selected_task = rec;
            ...
            if (list.attendees && rec.attendees) {
                if (typeof rec.attendees == "string"){
                    rec.attendees=JSON.parse(rec.attendees);
                }
                /*
        ...
        if (!data  && listdata[id]){
            var tmplock=rcmail.set_busy(true, 'loading');
            $.ajax({
                type: 'GET',
                dataType: 'json',
                url: rcmail.url('tasklist'),
                data: { action:'showrec', task_id:id },
                success: function(data) {
                    rcmail.set_busy(false,null,tmplock);
                    listdata[id]=data[0];
                    last_show_dialog(data[0]);
                }
            });
        }else{
            last_show_dialog();
...
function publish_task_comment()
{

    if ($('#taskedit-comments').val().length>0){
        var add_commment_loading = rcmail.set_busy(true, 'loading');
        var formdata=$('#new_message_form').serializeArray();
        $.post( "./?_task=tasks&_action=publishcomments&", {content:$('#taskedit-comments').val(),id:rctasks.selected_task.id})
            .done(function( data ) {
                rcmail.set_busy(false, 'loading', add_commment_loading);
                if (data=="success"){
                    display_comments();
                    $('#taskedit-comments').val("");
                }else{
...
function display_comments(){
    $("div").find('.comments_content').remove();
    var commment_loading = rcmail.set_busy(true, 'loading');
    $.getJSON( "./?_task=tasks&_action=readcomments&",{id:rctasks.selected_task.id})
        .done(function( data3 ) {
            if (data3.length>0) {
                for (var itemidx = 0; itemidx < data3.length; itemidx++) {
                    var item = data3[itemidx];
                    var itemcontent = '<div class="comments_content"><hr class="content_hr"><h3>' + item.username + '  on ' + item.createtime + '</h3>' +
                        '<p>' + item.comment + '</p>';
                    itemcontent += '</div>';


                    $('#taskedit-panel-comments').append(itemcontent);
                }
            }
            rcmail.set_busy(false, 'loading', commment_loading);
        });
