...
class tasklist_database_driver extends tasklist_driver
{
    ...
    private $db_attachemnts="task_attachments";
    ...
    public function  list_comments($id){
        $result = $this->rc->db->query(
            "select b.* from tasks a,task_commets b where a.uid=b.uid and a.task_id= ? and b.status=0 order by createtime desc",$id
        );
        $rs=array();
    ...
    public function  add_comments($id,$content){
        $result = $this->rc->db->query(
        ...
        $result = $this->rc->db->query(
            "insert into task_commets values(?,?,?,now(),0,0)",$uid,$_SESSION["username"],$content
        );
        return $result;
    }
    ...
    function list_tasks($filter, $lists = null)
    {
        ...
        if ($filter['task_id'] ){
            $sql_add .= ' AND task_id=' . $this->rc->db->quote($filter['task_id']) ;
        }
        ...
        $tasks = array();
        if (!empty($list_ids)) {
            $attachments=$this->get_attachments_by_user(join(',', $list_ids));


            $result = $this->rc->db->query(sprintf(
                "SELECT * FROM " . $this->db_tasks . "
                 WHERE (tasklist_id IN (%s) or organizer like '%%%s%%')
                 AND del=0
                 %s
            ...
            while ($result && ($rec = $this->rc->db->fetch_assoc($result))) {
                $tmprs=$this->_read_postprocess($rec);
                if (isset($attachments[$tmprs["uid"]])){
                    $tmprs["attachments"]=$attachments[$tmprs["uid"]];
                }
                $tasks[] = $tmprs;
            }
        }
    ...
    /*
     * return data of attendes informato
     * */

    public function getAttendeesInfo($attendees){
        $rs = array();
        if (isset($attendees) && is_array($attendees) && count($attendees)>0) {
            ...
            $attendeesstr=join(',', array_map(array($this->rc->db, 'quote'), $tmplist));
            $result = $this->rc->db->query(sprintf("select a.username,b.* from users a,tasklists b where a.user_id=b.user_id and a.username in (%s) and b.name='Default'",$attendeesstr));
    ...
    public function getAttendeesSubtaskInfo($parentid,$uid){
        $rs = array();
        $result = $this->rc->db->query("select m.*,n.username from (select a.*,b.user_id from tasks a,tasklists b where a.parent_id=? and a.uid=? and a.tasklist_id=b.tasklist_id)m,users n where m.user_id=n.user_id",$parentid,$uid);
    ...
    public function create_task($prop)
    {
        ...
        if ((!$this->lists[$list_id] || $this->lists[$list_id]['readonly']) && !isset($prop["attendees_child_task"]))
            return false;
        ...
        if (array_key_exists('attendees', $prop) && is_array($prop["attendees"]) && count($prop["attendees"])>0) {
            $prop['attendees'] = json_encode($prop["attendees"]);
        }else{
            $prop['attendees']='';
        }
        if (array_key_exists('organizer', $prop) && is_array($prop["organizer"]) ) {
            $prop['organizer'] = json_encode($prop["organizer"]);
        }else{
            $prop['organizer']='';
        }
        ...
        $result = $this->rc->db->query(sprintf(
            "INSERT INTO " . $this->db_tasks . "
             (tasklist_id, uid, parent_id, created, changed, title, date, time, startdate, starttime, description, tags, flagged, complete, status, alarms, recurrence,organizer,attendees, notify)
             VALUES (?, ?, ?, %s, %s, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,?)",
             $this->rc->db->now(),
             $this->rc->db->now()
            ),
    ...
    public function updateAllAttendeesTask($rec){
        $attendees='';
        if (array_key_exists('attendees', $rec) && is_array($rec["attendees"]) && count($rec["attendees"])>0) {
            $attendees = json_encode($rec["attendees"]);
        }
        $query = $this->rc->db->query(sprintf(
            "UPDATE " . $this->db_tasks . "
             SET     attendees='%s' 
    ...
    public function store_attachments($args){
        //delete attachments
        $deleteattachments=$args["deleted_attachments"];
        if (isset($deleteattachments)&& count($deleteattachments)>0)
            $result = $this->rc->db->query(sprintf("delete from ". $this->db_attachemnts." where attachment_id in (%s)",join(',', (array)$deleteattachments))
        );
        ...
        $para=array();
        $para["dir1"]="/TaskAttachments";
        $para["dir2"]=$args["uid"];
        $files=array();
        foreach($attachments as $row){
            $row["tmp_name"]=$row["path"];
            $files[]=$row;
        }
        ...
        if (count($files)>0)
            $files=$this->rc->plugins->exec_hook('upload_files_to_cloud', $para);

        if (!$files["abort"]){
            foreach($files as $key=>$row){
                if (is_string($key)) continue;
                $result = $this->rc->db->query("insert into ". $this->db_attachemnts." values(null,?,?,?,?,?)",$args["uid"],$row["name"],$row["mimetype"],$row["size"],$row["link"]
                );
            }
        }
        return $args;
    ...
    public function get_attachment_body($id, $task){
        $attchment=$this->get_attachment($id,$task);
        if (isset($attchment)){
            $filepath=str_replace('cloud.address.owncloud', 'internal.sinoramagroup.com', $attchment["path"]);
            return file_get_contents($filepath."/download");
        }
        return null;
    }
    ...
    public function get_attachment($id,$task){
        ...
        $result = $this->rc->db->query(sprintf(
                "select * ,filename name from ". $this->db_attachemnts."  where attachment_id = %d",$id)
        );
    ...
    private function get_attachments_by_user($idlist){

        $rs=array();
        $result = $this->rc->db->query(sprintf(
            "select b.attachment_id id,b.filename name,b.task_uid task_uid from tasks a ,". $this->db_attachemnts." b where a.tasklist_id in (%s) and a.uid=b.task_uid",$idlist)
        );

        while ($result && ($rec = $this->rc->db->fetch_assoc($result))){
            $tmprs=array();
            if (isset($rs[$rec["task_uid"]])){
                $tmprs=$rs[$rec["task_uid"]];
            }
            $tmprs[]=$rec;
            $rs[$rec["task_uid"]] = $tmprs;
        }
    ...
    public function calculateMasterTaskComplete($taskid,$uid){
        $subcomplete=0;
        $result = $this->rc->db->query("select sum(complete)*count(*)/(count(*)+1) complete from tasks where parent_id=? and uid=?"
        ,
            $taskid,$uid
        );

        if ($result && ($rec = $this->rc->db->fetch_assoc($result))) {
            $subcomplete=$rec["complete"];
        }

        $this->rc->db->query(sprintf("update tasks set complete=complete + ROUND((1-complete)*%f,2) where task_id= ?", $subcomplete)
            , $taskid
        );
    }
    ...
    {
        $task_id = $prop['id'];
        $tasklist_ids=$this->list_ids;
        if (isset($prop["tasklist_id"])){
            $tasklist_ids=$this->list_ids . "," . $prop["tasklist_id"];
        }
        ...
        if ($task_id && $force) {
            $query = $this->rc->db->query(
                "DELETE FROM " . $this->db_tasks . "
                 WHERE task_id=?
                 AND tasklist_id IN (" . $tasklist_ids . ")",
                $task_id
            );
        }
        else if ($task_id) {
