...
class tasklist extends rcube_plugin
{
    ...
    public function startup($args)
    {
        ...
        if ($args['task'] == 'tasks' && $args['action'] != 'save-pref') {
            ...
            $this->register_action('readcomments', array($this, 'get_comments'));
            $this->register_action('publishcomments', array($this, 'publishcomments'));
    ...
    public function publishcomments(){
        ...
        $content=rcube_utils::get_input_value('content', rcube_utils::INPUT_GPC);
        $id=rcube_utils::get_input_value('id', rcube_utils::INPUT_GPC);
        $this->driver->add_comments($id,$content);
        echo "success";
        exit;
    }
    ...
    public function get_comments(){
        $id=rcube_utils::get_input_value('id', rcube_utils::INPUT_GPC);
        $rs=$this->driver->list_comments($id);
        echo json_encode($rs);
        exit;
    ...
    public function task_action()
    {
        ...
        case 'new':
            ...
            if ($success = $this->driver->create_task($rec)) {
                $this->driver->add_comments($success,"Create task");
                //add a sub task for every attendees
                $attendeesinfo=$this->driver->getAttendeesInfo($rec["attendees"]);
                if (isset($attendeesinfo) && count($attendeesinfo)>0){
                    $rec2=$rec;

                    $rec2["parent_id"]=$success;
                    $rec2["attendees_child_task"]="yes";
                    $noticeemail=array();
                    foreach($rec["attendees"] as $row2){
                        $row=$attendeesinfo[$row2["email"]];
                        $noticeemail[]=$row2["email"];
                        $rec2["list"]=$row["tasklist_id"];
                        $rec2["title"]="[". $row2["name"]."]--" . $rec["title"];
                        $this->driver->create_task($rec2);

                    }

                    $para=array();
                    $para["msg_content"]="New tasks:" . $rec2["title"];
                    $para["msg_receiver"]=implode(",",$noticeemail);
                    $para["status"]="sending";
                    $this->rc->plugins->exec_hook('send_sinorama_notices', $para);
                }

                $this->driver->store_attachments($rec);
        ...
        case 'complete':
            ...
            $this->driver->add_comments($rec['id'],"Complete the task");
        ...
        case 'edit':
            ...
            if ($success = $this->driver->edit_task($rec)) {
                ...
                $this->driver->add_comments($rec['id'],"Edit task");
                ...
                if(!isset($rec["parent_id"])) {
                    //for master task only
                    //deal with the attendees's sub task
                    $attendeessubtask = $this->driver->getAttendeesSubtaskInfo($rec["id"], $rec["uid"]);

                    $attendeesinfo = $this->driver->getAttendeesInfo($rec["attendees"]);

                    //check new attendees.
                    if (isset($attendeesinfo) && count($attendeesinfo) > 0) {
                        $rec2["parent_id"] = $rec['id'];
                        $rec2["attendees_child_task"] = "yes";
                        $noticeemail=array();
                        foreach ($rec["attendees"] as $row2) {
                            if (!isset($attendeessubtask[$row2["email"]])) {
                                //this means add new attendees
                                $rec2 = $rec;
                                $row = $attendeesinfo[$row2["email"]];
                                $noticeemail[]=$row2["email"];
                                $rec2["list"] = $row["tasklist_id"];
                                ...
                                //check the task delete status
                                if ($attendeessubtask[$row2["email"]]["del"] == 1) {
                                    $this->driver->undelete_task(array(id => $attendeessubtask[$row2["email"]]["task_id"], tasklist_id => $attendeessubtask[$row2["email"]]["tasklist_id"]));
                                }
                                unset($attendeessubtask[$row2["email"]]);
                            }
                    ...
                    //soft delete task
                    foreach ($attendeessubtask as $key => $row) {
                        $this->driver->delete_task(array(id => $row["task_id"], tasklist_id => $row["tasklist_id"]), false);
                    }
                    //update all attendees information
                    $this->driver->updateAllAttendeesTask($rec);
                    $this->cleanup_task($rec);
                }
                //update master task complete status
                if (isset($rec["parent_id"])){
                    $task_id=$rec["parent_id"];
                    $this->driver->calculateMasterTaskComplete($task_id,$rec["uid"]);
                }
          ...
          case 'move':
              foreach ((array)$rec['id'] as $id) {
                  ...
                  if ($this->driver->move_task($r)) {
                      $this->driver->add_comments($rec['id'],"Move the task");
        ...
        case 'delete':
            ...
            if ($success = $this->driver->delete_task($rec, false)) {
                $this->driver->add_comments($rec['id'],"Delete the task");
    ...
    public function tasklist_action()
    {
        ...
        case 'search':
            ...
            return;
        case 'showrec':
            $task_id  = rcube_utils::get_input_value('task_id', rcube_utils::INPUT_GPC);

            $filter = array('task_id' => $task_id);
            $data = $this->tasks_data($this->driver->list_tasks($filter));
    ...
    */
    ...
    public function attachment_get()
    {
        ...
        $tid = rcube_utils::get_input_value('_t', rcube_utils::INPUT_GPC);
        ...
        $task = array('id' => $tid, 'list' => $list, 'rev' => $rev);
